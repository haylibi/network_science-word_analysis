...
class EvaluateNetworks():
    ...
    def get_shortest_paths(self, force_update=False, print_tqdm=True, **kwargs):
        if not hasattr(self, 'shortest_paths') or force_update:
            ...
            if print_tqdm: _iter = tqdm.tqdm(_iter, desc='    Finding Paths', total=len(self.network.nodes), file=sys.stdout)
    ...
    def z_score(self, motif_sizes=[3], force_update=False, remove_gtrie_dir=False, z_score_k=None, debug=False, **kwargs):
        # If method has already been called, no need to calculate values again  (unless forceupdate is true)
        if not all(x in self.motif_sizes for x in motif_sizes) or force_update:
            # Install G_Trie
            _install_g_trie(directory=self.gtrie_dir)

            output = {}
            for motif_size in motif_sizes:
                if motif_size in self.motif_sizes:
                    output[f'Most Relevant Subgraph (size={motif_size})'] = f"{df.iloc[0]['motif']} with a Z-Score of {df.iloc[0][' z_score']}"
                    continue
                ...
                df['motif'] = df['adjmatrix'].apply(save_graph_with_labels)

                self.z_scores = pd.concat([self.z_scores, df])
                
                output[f'Most Relevant Subgraph (size={motif_size})'] = f"{df.iloc[0]['motif']} with a Z-Score of {df.iloc[0][' z_score']}"
                self.motif_sizes.append(motif_size)
    ...
    def motif_frequency(self, motif_sizes=[3], force_update=False, **kwargs):
        if not all(x in self.motif_sizes for x in motif_sizes) or force_update:
            self.z_score(motif_sizes=motif_sizes, **kwargs)
