...
bool CmdLine::create;
bool CmdLine::raw; // added for NetSci homework
...
void CmdLine::compute_results() {
  ...
  // added for NetSci homework
  if (raw) {
    FILE *fraw = fopen("raw.txt", "w");
    fprintf(fraw, "adjmatrix, occ_original, z_score, avg_random, stdev_random\n");    
    for (i=0; i<m_original.size(); i++) {
      fprintf(fraw, "\"%s\", %d, %.9lf, %.9lf, %.9lf\n",
	      res[i].s, res[i].f_original,
	      isnan(res[i].z_score)?0:res[i].z_score,
	      res[i].avg_random, res[i].dev_random);
    }

    printf("Raw results written to file \"raw.txt\" [NetSci Homework]\n");    
    fclose(fraw);
  }
...
void CmdLine::defaults() {
  ...
  raw = false; // added for NetSci homework
}
